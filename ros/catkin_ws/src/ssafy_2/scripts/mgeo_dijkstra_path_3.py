...
class dijkstra_path_pub :
    def __init__(self):
        ...
        destination = r.get('cmd')
        self.end_index = int(destination)
        ...
        self.start_node = self.find_near_node(self.status_msg.position.x, self.status_msg.position.y)
        ...
        while not rospy.is_shutdown():
            value = r.get('status')
            if value == 'stop':
                self.end_node = 'A119BS010226'
                self.global_path_msg = self.calc_dijkstra_path_node(self.start_node, self.end_node)
                r.set('status', '0')
            
            if self.is_stop and self.cnt == 0:
                self.cnt = 1
                load_path = os.path.normpath(os.path.join(current_path, 'lib/mgeo_data/R_KR_PG_K-City1'))
                mgeo_planner_map = MGeo.create_instance_from_json(load_path)
                node_set = mgeo_planner_map.node_set
                link_set = mgeo_planner_map.link_set
                self.nodes=node_set.nodes
    ...
    def find_near_node(self, x, y):
        tmp_idx = None
        min = 21e8
        for node_idx in self.nodes:
            dis = sqrt(pow(x - self.nodes[node_idx].point[0], 2) + pow(y - self.nodes[node_idx].point[1], 2))
            if dis < min:
                min = dis
                tmp_idx = self.nodes[node_idx].idx
    ...
    # MySQL 연결 설정
        ...
        # 예제 쿼리 실행 (여기서는 데이터베이스의 'your_table'을 사용하는 예시)
        cursor.execute("SELECT id,node_number FROM desti")
        ...
        for row in result:
            if row[0] == self.end_index:
                self.end_node = row[1]
                rospy.loginfo(row[1])  # ROS 로그에 출력
